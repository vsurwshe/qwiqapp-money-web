...
class BillAttachmentsApi extends AbstractApi {
  ...
  createBillAttachment(success, failure, profileId, billId, data) {
    this.process(success, failure, profileId + "/bills/" + billId + "/attachments", this.methodType.POST, data);
  }

  getBillAttachments(success, failure, profileId, billId) {
    this.process(success, failure, profileId + "/bills/" + billId + "/attachments", this.methodType.GET, profileId);
  }

  getBillAttachmentsById(success, failure, profileId, billId, attachmentId) {
    this.process(success, failure, profileId + "/bills/" + billId + "/attachments/" + attachmentId, this.methodType.GET);
  }

  deleteBillAttachment(success, failure, profileId, billId, attachmentId) {
    this.process(success, failure, profileId + "/bills/" + billId + "/attachments/" + attachmentId, this.methodType.DELETE, profileId);
  }
