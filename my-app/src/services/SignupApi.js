...
class SignupApi {
  ...
  getToken = () => {
    new LoginApi().login("dummy@email.com","dummyPwd",() => {
      console.log("Your Token is", Store.getAccessToken());}, () => {console.log("Cantnot Fetch the Admin Token");});
  };
  //Registers User
  ...
  async existsUser(success, failure, data) {
    ...
    setTimeout(()=>{
      ...
      instance.request().then(resp => { if (resp.data) { validResponse(resp,success) } })
