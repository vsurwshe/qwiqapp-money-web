...
class CreateLable extends Component {
  ...
  //this is geting all the lables and stroed 
  componentDidMount() {
  ...
  handleSubmit = e => {
    ...
    new LabelApi().createLabel(
      () => {this.setState({ labelCreated: true });this.callAlertTimer("success", "New Label Created!!");},
      this.errorCall,
      11,
      data
    );
              ...
              <FormGroup>
                <h5><b>CREATE LABEL</b></h5>
                 ...
                 <FormGroup check className="checkbox">
                        <Label check className="form-check-label" htmlFor="checkbox1">Enable this as Sub-Label</Label><br/>
      ...
      <Collapse isOpen={this.state.collapse}>
        <Input type="select" name="selectLg" id="selectLg" onChange={(event)=>this.setState({parentId:event.target.value})} bsSize="lg">
          ...
          {this.state.labels.map((labels) => {
               return(<option key={labels.id} value={labels.id}>{labels.name}</option>)
          })}
         </Input>
      </Collapse>
    );
