  ...
  categories : function(categories){
    ...
    categories.map(category => {
           if (category.subCategories !== null){
             options.push({ value:category.id, label:<b>{category.name}</b>, color:category.color ===null || category.color ==="" ?"#000000": category.color})
             category.subCategories.map(subCategory=>{
               return options.push({label:<span style={{paddingLeft:30}}>{subCategory.name}</span> , color:subCategory.color ===null || subCategory.color ==="" ?"#000000": subCategory.color,value:subCategory.id})
             })
           } else{
             return options.push({value: category.id, label: <b>{category.name}</b>, color: category.color ===null || category.color ===""  ?"#000000" :category.color })
           }
           return 0;
       })
    return options;
