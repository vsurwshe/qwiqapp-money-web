// This const Variable can change according to Environment
...
const whyBillsReminder = [
    ...
    {index: 4, header: "Smart Bills Reminder", content: "Tracks all your bills and classify into upcoming, overdue, paid and unpaid bills. Sends you a push notification to remind you of your upcoming bills."},
...
function load(){
    ...
    // This Lines are changeing href tags dynamically in according to elements
    ...
    // This Lines are showing features card dyanamically  
    var elements='';
    for(var i = 0, length = products.length; i < length; i++) {
        elements = elements+"<div class='plan col-12 mx-2 my-2 justify-content-center col-lg-4'> <div class='plan-header text-center pt-5'> <h3 class='plan-title mbr-fonts-style display-5'>"
        ...
        products[i].features.forEach((element)=>{ 
           return elements = elements+"<li>"+element+"</li>";
        })
        elements = elements+"</ul></ul></div></div></div>";
    }
    // This line appendig above elemenst string to features Section in index.html 
    $('#featuresSection').append(elements);



    // =========== Why Choose Bills Reminder? content load from hear ============
    let brElement = '';
    whyBillsReminder.forEach(billsReminder=>{
        brElement = brElement+ "<div class='card separline pb-4'><div class='step-element d-flex'><div class='step-wrapper pr-3'><h3 class='step d-flex align-items-center justify-content-center'>"
    ...
    $('#whyBillsReminder').append(brElement);
    // =============== END =================
}